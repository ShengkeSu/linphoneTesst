...
public class LinphonePreferences {
	...
	public static class AccountBuilder {
		...
		public void saveNewAccount() throws LinphoneCoreException {
			...
			if (tempUserId==null)
				tempUserId = tempUsername+"@conf.shec.edu.cn";


			if(tempRealm != null)
				prxCfg.setRealm(tempRealm);
			android.util.Log.i("aaaLinphonePreferences","name="+tempUsername+"\nuserid="+tempUserId+"\npassword="+tempPassword
														+"\nha1="+tempHa1+"\ntempRealm="+tempRealm+"\nDomain="+tempDomain);
//			LinphoneAuthInfo authInfo = LinphoneCoreFactory.instance().createAuthInfo(identity, tempUserId, tempPassword, tempHa1, tempRealm, tempDomain);
	...
	public void setAccountUserId(int n, String userId) {
		...
		if(info != null) {
			...
			saveAuthInfo(info);
			android.util.Log.i("aaa","saveAuthInfo=="+info.getUserId());
		}
	}
